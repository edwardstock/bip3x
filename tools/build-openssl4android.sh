...
LIB_NAME="OpenSSL_1_1_0h"
LIB_DEST_DIR=${TOOLS_ROOT}/libs
#[ -d ${LIB_DEST_DIR} ] && rm -rf ${LIB_DEST_DIR}
[ -f "${LIB_NAME}.tar.gz" ] || wget https://github.com/openssl/openssl/archive/${LIB_NAME}.tar.gz;
# Unarchive library, then configure and make for specified architectures
configure_make() {
  ARCH=$1; ABI=$2;
  rm -rf "${LIB_NAME}"
  tar xfz "${LIB_NAME}.tar.gz"
  pushd "openssl-${LIB_NAME}"

  configure $*

  #support openssl-1.0.x
  if [[ $LIB_NAME != OpenSSL_1_0_* ]]; then
    if [[ $ARCH == "android64" ]]; then
