...
bip3x::ecdsa_recoverable_signature bip3x::sign_message(const bytes_data& msg, const bytes_32& priv_key) {
    return sign_message(msg.data(), msg.size(), priv_key.cdata());
}

bip3x::ecdsa_recoverable_signature bip3x::sign_message(const uint8_t* msg, size_t msg_len, const uint8_t* priv_key) {

    bytes_array<65> sig_result;
    uint8_t pby_result;

    ecdsa_sign(&secp256k1, HASHER_SHA3K, priv_key, msg, msg_len, sig_result.data(), &pby_result, nullptr);
...
bip3x::bytes_32
bip3x::mnemonic_to_private_key(const std::string& mnemonic_words, const bip3x::derivation_path& derivation) {
