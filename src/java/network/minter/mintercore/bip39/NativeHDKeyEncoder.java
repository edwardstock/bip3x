...
public final class NativeHDKeyEncoder {
    ...
    public static HDKey makeExtenderKey(HDKey rootKey) {
        return makeExtenderKey(rootKey, MINTER_DERIVATION_PATH);
    }

    public static HDKey makeExtenderKey(HDKey rootKey, String derivationPath) {
        ...
        return encoderMakeExtendedKey(rootKey, derivationPath);
    }

    private static native HDKey encoderMakeBip32RootKey(ByteBuffer seed, BTCNetwork net);

    private static native HDKey encoderMakeExtendedKey(HDKey rootKey, String derivationPath);
