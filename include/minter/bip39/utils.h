...
class Data : public toolbox::data::bytes_data {
public:
    Data()
        : bytes_data() {
    }
    Data(const bytes_data& other)
        : bytes_data(other) {
    }
    Data(bytes_data&& other)
...
public:
    ...
    FixedData(const uint8_t* data, size_t len)
        : FixedData() {
        if (data != nullptr) {
    ...
    /// \param data
    explicit FixedData(const uint8_t* data)
        : FixedData(data, N) {
    }
    ...
    FixedData(const std::string& hexString)
        : FixedData() {
        if (hexString.length() % 2 == 0 && hexString.length() <= N * 2) {
            auto tmp = toolbox::data::hex_to_bytes(hexString);
    ...
    /// \brief converts hex string to bytes.
    /// \param hexString
    FixedData(const char* hexString)
        : FixedData(hexString) {
    }

    explicit FixedData(const std::vector<uint8_t>& data)
        : FixedData() {
        size_t readLen = std::min(data.size(), N);
        std::copy(data.begin(), data.begin() + readLen, begin());
    }

    explicit FixedData(std::vector<uint8_t>&& data)
        : FixedData() {
        size_t readLen = std::min(data.size(), N);
        std::copy(data.begin(), data.begin() + readLen, begin());
        data.clear();
    ...
    FixedData(const FixedData<N>& other)
        : FixedData() {
        clear();
        m_data = other.m_data;
    }
